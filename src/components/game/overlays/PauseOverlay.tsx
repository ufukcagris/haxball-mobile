...
export function PauseOverlay({ onResume, onMenu, onLobby }: PauseOverlayProps) {
  ...
  return (
    <div
      className='absolute inset-0 flex items-center justify-center flex-col p-4 z-100'
      style={{
      ...
      {showConfirmExit ? (
        <div className='flex flex-col items-center'>
          ...
          <div className='flex flex-col gap-3 w-[220px]'>
            ...
            <OverlayButton onClick={() => setShowConfirmExit(false)}>
              HAYIR, DEVAM ET
            </OverlayButton>
          </div>
        </div>
        ...
        <>
          <div className='text-[clamp(1.4rem,4vw,2.2rem)] font-black tracking-tight mb-6 text-(--accent) uppercase'>
            ⏸ DURAKLATILDI
          </div>
          ...
          <div className='flex flex-col gap-3 w-[220px]'>
            ...
            {onLobby && myRole !== 'solo' && (
              <OverlayButton onClick={onLobby}>LOBIYI AC</OverlayButton>
            )}
            <OverlayButton
          ...
          {myRole !== 'solo' && (
            <>
              <div className='mt-8 text-[0.6rem] text-white/40 uppercase tracking-[2px] font-bold'>
                Takimlar
              </div>
              <IngameLobbyOverlay />
            </>
