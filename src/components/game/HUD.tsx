...
export function HUD() {
  const { scoreRed, scoreBlue, timeLeft, overtime, time, redNick, blueNick, paused } = useGameStore();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        <div className="flex items-center gap-1.5">
          <div className="font-['Share_Tech_Mono',monospace] text-[1.1rem] text-[var(--text)] tracking-[2px]">
            {formatTime()}
          </div>
          {overtime && (
            <div className="bg-[rgba(255,214,0,0.2)] border-[1.5px] border-[var(--yellow)] rounded-md
              px-2 py-px text-[0.65rem] font-black tracking-[2px] text-[var(--yellow)] uppercase">
              OT
            </div>
          )}
        </div>
        <div className="flex gap-1 mt-0.5">
          <button
            className="bg-transparent border-[1.5px] border-[var(--border)] rounded-[7px] text-[var(--text-dim)]
